...
#### E2E環境でのViteアセット読み込みについて
- **問題**: `APP_ENV=testing`では`@vite`ディレクティブが本番モードで動作し、Vite開発サーバーがない環境で500エラーが発生
- **解決策**: `.env.ci`で`APP_ENV=local`を使用し、`app.blade.php`でビルド済みアセット（manifest.json）の存在を明示的にチェック
- **動作**: ビルド済みアセットがあれば使用（CI環境）、なければVite開発サーバーを使用（ローカル開発）
